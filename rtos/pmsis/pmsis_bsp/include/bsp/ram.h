...
static inline int pi_ram_ioctl(struct pi_device *device, uint32_t cmd, void *arg);

static inline int pi_ram_id_alloc(struct pi_device *device);
...
{
    int (*ioctl)(struct pi_device *device, uint32_t cmd, void *arg);
...
int pi_ram_ioctl(struct pi_device *device, uint32_t cmd, void *arg)
{
  pi_ram_api_t *api = (pi_ram_api_t *)device->api;
  return api->ioctl(device, cmd, arg);
}

int pi_ram_id_alloc(struct pi_device *device)
{
  pi_ram_api_t *api = (pi_ram_api_t *)device->api;
